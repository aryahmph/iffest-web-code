...
function PopularDestination(props) {
    return (
        <section className="container mt-28">
            <h4 className="mb-8 text-secondary font-semibold text-xl lg:text-2xl">Popular Destination</h4>
            <div
                    ...
                    props.data.map((item, index) => {
                        return (
                            <div key={item.id.toString()}
                                 className={`${index === 0 ? "col-span-2" : ""} border rounded-lg overflow-hidden relative transition hover:opacity-90 active:opacity-90`}>
                                <Button type="link" href={`${item.slug}`} className="group">
                                    ...
                                    <div
                                        className={`${index >= 2 ? "ml-5 mb-5" : "ml-8 mb-8"} w-2/3 absolute bottom-1.5 text-white`}>
